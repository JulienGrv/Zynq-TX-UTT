...
typedef struct {
    ...
    XDohist_uio_map maps[ MAX_UIO_MAPS ];
} XDohist_uio_info;

/***************** Variable Definitions **************************************/
static XDohist_uio_info uio_info;
...
static int uio_info_read_name(XDohist_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDohist_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDohist_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDohist_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDohist_Initialize(XDohist *InstancePtr, const char* InstanceName) {
	XDohist_uio_info *InfoPtr = &uio_info;
